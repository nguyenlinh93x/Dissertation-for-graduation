...
public class TimeSeries {
		...
		try {
													...
													// file
			{
				if (line.length() > 0) // ignore empty lines
				{
					...
					// throw new InternalError("ERROR:  Line " +
					// (tsArray.size()+1) +
					...
					int currentCol = 0;
					final String currentToken = lines[3];
	...
	private static boolean determineIsFirstColTime(String filePath) {
		...
		try {
			...
			final Vector possibleTimeValues = new Vector(NUM_OF_VALUES_TO_CMP); // 'stores
																				// numOfValuesToCompare'
																				// values
			...
			final double expectedDiff = ((Double) possibleTimeValues.get(1))
					.doubleValue()
			...
			for (int x = 1; x < possibleTimeValues.size(); x++) {
				if (Math.abs(((Double) possibleTimeValues.get(x)).doubleValue()
						- ((Double) possibleTimeValues.get(x - 1))
								.doubleValue() - expectedDiff) > Math
						.abs(flexibility)) {
					return false;
				}
			} // end for loop
